...
Avoid CMLSPROJECTJUCEAudioProcessor::oscMessageReceived(const juce::OSCMessage& message)
{
    if (message.getAddressPattern().toString() == "/delay" && message.size() == 1 && message[0].isFloat32())
    {
        float val = message[0].getFloat32();
        
        DBG("DELAY CHANGE");
        //parameters.getParameter("delay")->setValueNotifyingHost(val);
    }
    else if (message.getAddressPattern().toString() == "/distortion" && message.size() == 1 && message[0].isFloat32())
    {
        float val = message[0].getFloat32();
        
        DBG("DISTORTION CHANGE");
        //parameters.getParameter("reverb")->setValueNotifyingHost(val);
    }
    else if (message.getAddressPattern().toString() == "/reverb" && message.size() == 1 && message[0].isFloat32())
    {
		float val = message[0].getFloat32();

        DBG("REVERB CHANGE");
		//parameters.getParameter("distortion")->setValueNotifyingHost(val);
	}
    else
    {
        DBG("Messaggio OSC non valido o non riconosciuto");
    }
}